class Baozi extends ComicSource {
    ...
    // 更新链接
    url = "https://cdn.jsdelivr.net/gh/nukbodu/mh-configs@main/baozi.js"
    ...
    comic = {
        ...
        loadEp: async (comicId, epId) => {
			...
			const images1 = doc1.querySelectorAll("ul.comic-contain > div > amp-img").map(e => e.attributes['src']);
			images.push(...images1);
		...
		//		try {
		//			// 动态生成分页URL
		//			let urlSuffix = page === 0 ? 
		//				`0_${epId}.html` : 
		//				`0_${epId}_${page}.html`;
		//			
		//			let res = await Network.get(
		//				`https://cn.bzmanga.com/comic/chapter/${comicId}/${urlSuffix}`
		//			);
		//	
		//			// 终止条件：遇到404或非200状态码
		//			if(res.status === 404 || res.status !== 200) break;
		//
		//			// 解析当前页图片
		//			let document = new HtmlDocument(res.body);
		//			let pageImages = document.querySelectorAll("ul.comic-contain > div > amp-img")
